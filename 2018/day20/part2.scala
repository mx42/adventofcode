...
println(s"Nb rooms: ${roomsAndDoors.size}")
...
def computeDists(remaining: List[Pos], todo: List[(Pos, Int)], distMap: Map[Pos, Int]): Map[Pos, Int] = todo match {
  ...
  case (node, dist) :: tail =>
    ...
    computeDists(remaining.filterNot(neighbors.contains), tail ++ neighbors.map(p => (p, dist + 1)), distMap + (node -> (dist / 2)))
}

val distMap = computeDists(roomsAndDoors.toList, List((Origin -> 0)), Map.empty[Pos, Int])

def isRoom: ((Pos, Int)) => Boolean = { case (p, _) => (p.x % 2 == 0) && (p.y % 2 == 0) }

println("Rooms 1000+ doors away: " + distMap.filter(isRoom).map(_._2).count(_ >= 1000))
